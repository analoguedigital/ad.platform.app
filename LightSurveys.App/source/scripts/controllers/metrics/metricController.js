...
angular.module('lm.surveys').controller('metricController', ['$scope', function ($scope) {
    ...
    if ($scope.metricGroup.isRepeater) {
        if ($scope.metricGroup.type === "IterativeRepeater")
            relatedFormValues = _.filter($scope.allFormValues, { 'metricId': $scope.metric.id, 'rowNumber': $scope.rowNumber });
        else
            relatedFormValues = _.filter($scope.allFormValues, { 'metricId': $scope.metric.id, 'rowNumber': $scope.rowNumber, 'rowDataListItemId': $scope.dataListItem.id });

        $scope.metric.inputName = _.camelCase('m' + $scope.metric.id) + $scope.rowNumber;
    }
    else {
        relatedFormValues = _.filter($scope.allFormValues, { 'metricId': $scope.metric.id });
